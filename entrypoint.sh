...
ACTION_STATE_FIND_PATTERN="${ACTION_STATE_FIND_PATTERN:-*.sls}"
...
if [ -n "${ACTION_STATE_FIND_PATH}" ]; then
  >&2 echo "==> Linting ${ACTION_STATE_FIND_PATH} for files ${ACTION_STATE_FIND_PATTERN}…"
  salt-lint ${SALT_LINT_EXTRA_PARAMS} `find "${ACTION_STATE_FIND_PATH}" -type f -name "${ACTION_STATE_FIND_PATTERN}"`

elif [ -d "${ACTION_STATE_PATH}" ]; then
  >&2 echo "==> Linting ${ACTION_STATE_PATH}…"
  salt-lint ${SALT_LINT_EXTRA_PARAMS} `find "${ACTION_STATE_PATH}" -type f -name 'init.sls'`
